...
# Wadamachi2Yokohama Route Calculator

和田町駅まで徒歩で移動し、そこから相鉄線を利用して横浜駅へ向かう場合の「最適な出発時間」と「乗車すべき電車」を算出するモジュールです。
文責:青柳/Gemini
...
2.  横浜駅への到着時刻順にソート
3.  和田町駅を**停車する電車のみ**を抽出し、徒歩時間を考慮してフィルタリング
...
## 関数仕様: `Wadamachi2Yokohama(paramJSON)`
...
| `ToWadamachi` | `number` | **Yes** | 現在地から和田町駅までの徒歩所要時間（分） | `14` |
...
| `station_name` | `string` | 乗車駅名 | 固定値: `"和田町"` |
| `fare` | `number` | 運賃（円） | 固定値: `188` (IC) |
| `station_walktime` | `number` | 駅までの徒歩時間（分） | 入力の `ToWadamachi` をパススルー |
| `station_departuretime` | `string` | 電車が和田町駅を発車する時間 | |
...
[
  {
    ...
    "traindata": {
      "trainNumber": "...",
      "departureTime": { "和田町": "12:30" }, // 停車する場合のみ値が入る
      "arrivalTime": "12:42",
      "ArrivalStatus": {
        "isReachable": true,
        "reachTime": "12:28",
...
2.  **通過列車の除外**: 相鉄線データの `departureTime["和田町"]` が存在しない（特急・快速などで通過する）列車は、計算対象から自動的に除外されます。
3.  **判定ロジック**:
      * `ユーザーが駅に着く時刻` = `currentTime` + `ToWadamachi`
      * `電車の発車時刻` \>= `ユーザーが駅に着く時刻` の場合のみ、有効なルートとして採用します。
4.  **出発時刻計算**:
      * UIに表示する「家を出る時間 (`departuretime`)」は、ギリギリに駅に着く時間ではなく、\*\*「電車の発車時刻から徒歩分数を引いた時間」\*\*として算出しています。
