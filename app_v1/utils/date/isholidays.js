 ...
 */
function getServiceIdsForDate(input = new Date()) {
    try {
        const date = parseToDate(input);
        if (!date) {
            // 無効な日付の場合は安全のため false (平日扱い) を返すように統一
            // 元のコードでは 'Weekday' 文字列を返していましたが、booleanに統一推奨です
            return false;
        }
...
// ESM default export: 関数そのものをエクスポート
export default getServiceIdsForDate;